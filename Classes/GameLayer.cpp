...
void GameLayer::playerScore(int player) {
    ...
    if (player == 1) {
        ...
        _ball->setNextPosition(ccp(_screenSize.width * 0.5f, _screenSize.height * 0.5 + _ball->radius()));
    } else {
        _player2Score++;
...
void GameLayer::ccTouchesMoved(cocos2d::CCSet *pTouches, cocos2d::CCEvent *event) {
    CCSetIterator i;
	CCTouch* touch;
    CCPoint tap;
    GameSprite * player;
...
void GameLayer::ccTouchesEnded(cocos2d::CCSet *pTouches, cocos2d::CCEvent *event) {
    CCSetIterator i;
	CCTouch* touch;
    GameSprite * player;
